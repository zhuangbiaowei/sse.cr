...
module HTTP::ServerSentEvents
  class Client
    ...
    def set_proxy(host : String, port : Int8)
    ...
    def stop : Nil
      LOG.info { "[#{@host}] Stop to listening event source" }
      @abort = true
    end

    def get(path : String, headers : HTTP::Headers? = nil) : Nil
      LOG.info { "[#{@host}] Start to listening event source" }
      loop do
        break if abort?

        LOG.info { "[#{@uri}] Preparing to send request to the endpoint : #{headers}" }
        @client.get(path, headers: prepare_headers(headers)) do |response|
    ...
    def post(path : String, headers : HTTP::Headers? = nil, body : HTTP::Client::BodyType? = nil) : Nil
      LOG.info { "[#{@host}] Start to listening event source" }
      loop do
        break if abort?

        LOG.info { "[#{@uri}] Preparing to send request to the endpoint : #{headers}" }
        @client.post(path, headers: prepare_headers(headers), body: body) do |response|
          case response.status
          when .ok?
            successful_response response
          when .found?, .see_other?, .temporary_redirect?
    ...
    private def prepare_headers(base_headers : HTTP::Headers) : HTTP::Headers
